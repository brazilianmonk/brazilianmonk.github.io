---
layout: page
title: Nominal Declensions — Charts & Words
permalink: /summaries/pali/nominal-declensions
---

- Words used for each declension
  - masc
    - a - purisa
    - ā - sā
    - i - aggi
    - ī - daṇḍī
    - u - bhikkhu
    - ū - sayambhū / sabbaññū
    - o - go
  - fem
    - ā - kaññā
    - i - ratti
    - ī - itthī / nadī
    - u - yāgu
    - ū - vadhū
  - nt.
    - a -
    - i - aṭṭhi
    - ī - sukhakārī (the dāna that brings happiness)
    - u - cakkhu
    - ū - gotrabhū
    - (o - cittagu)
      - rūpasiddhi's idea
      - other grammarians do not consider so
- Cittādigaṇa
- Pumādigaṇa
  - declension of yuva (m.) (pumādigaṇa) (to be added)
  - declension of addha (m.) (pumādigaṇa) (to be added)
- Rājādigaṇa
  - declension of brahma (rājādigaṇa) (to be added)
  - declension of sakhā (m.) (rajādigaṇa) (not for exam) (to be added)
- Satthādigaṇa
- Guṇavantādigaṇa
- Gacchantādigaṇa
  - karonta (to be added)
  - bhavanta (to be added)
- Rattādigaṇa
  - declension of sā (m.) (rattādigaṇa)

    | case | singular | plural |
    |---|---|---|
    | nom. | sā | sā, sāno |
    | voc. | (he) sa, sā | (he) " |
    | acc. | sānaṁ | sāne |
    | ins. / abl. | sānā | sānehi, sānebhi |
    | dat. / gen. | sāssa | sānaṁ |
    | loc. | sāne | sānesu |

    - mem aid:
      - the dog is healthy (sano)
      - long vocative form
    - note:
      - he is a more informal address
      - rūpasiddhi gives a different declension and says that paccakkhadhammā and gāṇḍīvadhanvā are declined as this
  - declension of go (m.) (rattādigaṇa - only this word)

| case | singular | plural |
|---|---|---|
| nom. | go | gāvo, gavo |
| voc. | (he) go | (he) gāvo, gavo |
| acc. | gāvaṁ, gavaṁ, gāvuṁ | gāvo, gavo |
| ins. | gāvena, gavena | gohi, gobhi |
| dat./gen. | gāvassa, gavassa | gavaṁ, gonaṁ, gunnaṁ |
| abl. | gāvasmā, gāvamhā, gāvā, gavasma, gavamhā, gavā | gohi, gobhi |
| loc. | gāvasmiṁ, gāvamhi, gāve, gavasmiṁ, gavasmiṁ, gave | gāvesu, gavesu, gosu |

    - mem aid:
      - both ā and a versions
      - acc. gāvuṁ
      - dat./gen.pl. gavaṁ, gonaṁ, gunnaṁ
  - declension of aṭṭhi (nt.) (rattādigaṇa)

    | case | singular | plural |
    |---|---|---|
    | nom. | aṭṭhi | aṭṭhī, aṭṭhīni |
    | voc. | (bho) aṭṭhi | (bhavantāni) aṭṭhī, aṭṭhīni |
    | acc. | aṭṭhiṁ | aṭṭhī, aṭṭhīni |

    - (the rest like aggi)
    - mem aid: close to aggi

  - declension of sukhakārī (nt.) (rattādigaṇa)

    | case | singular | plural |
    |---|---|---|
    | nom. | sukhakāri | sukhakārī, sukhakārīni |
    | acc. | sukhakāriṁ, sukhakārinaṁ | ... |

    - (the rest as daṇḍī)
    - note:
      - long vowel becomes short when neuter: sukharārī ⇒ sukhakāri dānaṁ

  - *declension of cakkhu (nt.) (rattādigaṇa)

    | case | singular | plural |
    |---|---|---|
    | nom. | cakkhu | cakkhū, cakkhūni |

    - (the rest like bhikkhu)
    - note:

  - declension of gotrabhū (nt.) (rattādigaṇa)

    | case | singular | plural |
    |---|---|---|
    | nom. | gotrabhu | gotrabhū, gotrabhūni |

    - (the rest like sayambhū)
    - note:

      - Some grammarians consider ‘cittagu’ as a neuter okāranta stem, since ‘gu’ sound at the end of that stem derives from ‘go’. In fact, it’s an ukāranta stem, because after the word formation, cittagu is ending with ‘u’. Therefore, a declension of neuter okāranta stems is not shown in this book.
- Sabbanāmagaṇa
  - declension of the pronoun 'pubba' (sabbanāmagaṇa)
    - masculine

      | case | singular | plural |
      |---|---|---|
      | nom. | pubbo | pubbe, pubbā |
      | voc. | (bho) pubba, pubbā | (bhonto) pubbe, pubbā |
      | acc. | pubbaṁ | pubbe |
      | ins. | pubbena | pubbehi, pubbebhi |
      | dat./gen. | pubbassa | pubbesaṃ, pubbesānaṁ |
      | abl. | pubbasmā, pubbamhā, pubbā | pubbehi, pubbebhi |
      | loc. | pubbasmiṁ, pubbamhi, pubbe | pubbesu |

      - mem aid: like sabba, but with some purisa forms in nom. and voc. pl., abl. sgl., and loc. sgl.
    - neuter

      | case | singular | plural |
      |---|---|---|
      | nom. | pubbaṁ | pubbe, pubbā, pubbāni |
      | voc. | (bho) pubba, pubbā | (bhavantāni) pubbe, pubbā, pubbāni |
      | acc. | pubbaṁ | pubbe, (pubbā?), pubbāni |

      - (the remaining as the masculine)
      - mem aid: like sabba, but with some purisa forms in nom. and voc. pl., abl. sgl., and loc. sgl.
    - feminine
      - as sabba
  - declension of the pronoun 'ya' (sabbanāmagaṇa)
    - as sabba in three genders, but without vocative forms
    - note: from ya onwards, the pronouns do not have vocative forms
  - declension of the pronoun 'ima' (sabbanāmagaṇa)
    - masculine

      | case | singular | plural |
      |---|---|---|
      | Nom. | ayaṁ | ime |
      | Acc. | imaṁ | " |
      | Ins. | iminā, anena | imehi, imebhi, ehi, ebhi |
      | Dat/Gen. | imassa, assa | imesaṁ, imesānaṁ, esaṁ, esānaṁ |
      | Abl. | imasmā, asmā, imamhā | imehi, imebhi, ehi, ebhi |
      | Loc. | imasmiṁ, asmiṁ, imamhi | imesu, esu |

      - mem aid: shortened forms with a- in sgl. and e- in pl. from acc. onwards
    - neuter

      | case | singular | plural |
      |---|---|---|
      | Nom./Acc. | idaṁ, imaṁ | ime, imāni |

      - (the rest as the masculine)
    - feminine

      | case | singular | plural |
      |---|---|---|
      | Nom. | ayaṁ | imā, imāyo |
      | Acc. | imaṁ | " |
      | Ins./Abl. | imāya | imāhi, imābhi |
      | Dat/Gen. | imāya, imissā, imissāya, assā, assāya | imāsaṁ, imāsānaṁ |
      | Loc. | imāyaṁ, imissaṁ, assaṁ | imāsu |

  - declension of the pronoun 'amu' (sabbanāmagaṇa)
    - masculine

      | case | singular | plural |
      |---|---|---|
      | Nom. | amu, asu | amū |
      | Acc. | amuṁ | " |
      | Ins. | amunā | amūhi, amūbhi |
      | Dat/Gen. | amussa | amūsaṁ, amūsānaṁ |
      | Abl. | amusmā, amumhā | amūhi, amūbhi |
      | Loc. | amusmiṁ, amumhi | amūsu |

      - mem aid: forms with a- in sgl. and forms with e- in pl.
      - note: amuka [amu+ka] is declined in purisādi, cittādi and kaññādi groups
    - neuter

      | case | singular | plural |
      |---|---|---|
      | Nom./Acc. | aduṁ | amū, amūni |

      - (the rest as the masculine)
    - feminine

      | case | singular | plural |
      |---|---|---|
      | Nom. | amu, asu | amū, amuyo |
      | Acc. | amuṁ | " |
      | Ins./Abl. | amuyā | amūhi, amūbhi |
      | Dat/Gen. | amuyā, amussā | amūsaṁ, amūsānaṁ |
      | Loc. | amuyaṁ, amussaṁ | amūsu |

      - note: no amuyā in loc.
  - declension of the pronoun 'eka' (sabbanāmagaṇa)
    - masculine

      | case | singular |
      |---|---|
      | nom. | eko |
      | acc. | ekaṁ |
      | ins. | ekena |
      | dat./gen. | ekassa |
      | abl. | ekasmā, ekamhā |
      | loc. | ekasmiṁ, ekamhi |
      | nom./acc. | ekaṁ |

    - neuter
      - (the rest as in masculine)
    - feminine

      | case | singular |
      |---|---|
      | nom. | ekā |
      | acc. | ekaṁ |
      | ins./abl. | ekāya |
      | dat./gen. | ekāya, ekissā |
      | loc. | ekāya, ekāyaṁ, ekassaṁ, ekissaṁ |

      - note: this is the only declension in sabbanāmagaṇa that has the form ekāya in loc.
    - 3 meanings of eka used in ekavacana and one in bahuvacana
      - declined as ekavacana
        - saṅkhyā: number (eko dve tayo; eko puriso āgacchati)
        - asadisa: unequalled, without equal (eko'mhi sammāsambuddho [I am the one and only fully self awakened, not simply 'one'])
        - asahāya: without a friend, alone (eko rāja nipajjāmi [I sleep alone, king])
      - declined as bahuvacana
        - ekacce: some (eke vadanti [some say]; appekacce) - declined as sabba
        - note: there are other meanings as well
  - declension of the pronoun 'ubha' (sabbanāmagaṇa) (same declension for the 3 genders, only plural)

    | case | singular |
    |---|---|
    | nom./acc. | ubho, ubhe |
    | ins./abl. | ubhohi, ubhobhi, ubhehi, ubhebhi |
    | dat./gen. | ubhinnaṁ |
    | loc. | ubhosu, ubhesu |

  - declension of the pronoun 'dvi' (sabbanāmagaṇa) (same declension for the 3 genders, only plural)

    | case | singular |
    |---|---|
    | nom./acc. | dve, duve |
    | ins./abl. | dvīhi, dvībhi |
    | dat./gen. | dvinnaṃ, duvinnaṃ |
    | loc. | dvīsu |

  - declension of the pronoun 'ti' (sabbanāmagaṇa) (only plural)
    - masculine

      | case | singular |
      |---|---|
      | nom./acc. | tayo |
      | ins./abl. | tīhi, tībhi |
      | dat./gen. | tiṇṇaṁ, tiṇṇannaṁ |
      | loc. | tīsu |
      | nom./acc. | tīṇi |

    - neuter
      - (the rest as the masculine)
    - feminine

      | case | singular |
      |---|---|
      | nom./acc. | tisso |
      | dat./gen. | tissannaṁ |

      - (the rest as the masculine)
  - declension of the pronoun 'catu' (sabbanāmagaṇa) (only plural)
    - masculine

      | case | singular |
      |---|---|
      | nom./acc. | cattāro (,caturo) |
      | ins./abl. | catūhi, catūbhi |
      | dat./gen. | catunnaṁ |
      | loc. | catūsu |
      | nom./acc. | cattāri |

    - neuter
      - (the rest as masculine)
    - feminine

      | case | singular |
      |---|---|
      | nom./acc. | catasso |
      | dat./gen. | catassannaṁ |

      - (the rest as masculine)
- Numbers 5-18 (aliṅgika)
- Number 19 and onwards are singular unless refering to collections (dve vīsatiyo)

{% include toc.html %}
